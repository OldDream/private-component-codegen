 ...
 * @LastEditTime: 2025-04-17 11:21:48
...
export async function searchSimilarEmbeddings(
  ...
  const results = await db
    .select({
      ...
      similarity: sql<number>`1 - (${openAiEmbeddings.embedding} <=> ${sql.raw(JSON.stringify(queryEmbedding))})`.as('similarity'),
    })
    .from(openAiEmbeddings)
    .where(
      sql`1 - (${openAiEmbeddings.embedding} <=> ${sql.raw(JSON.stringify(queryEmbedding))}) >= ${similarityThreshold}`
    )
